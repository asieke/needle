<script lang="ts">
	...
	let { probability, lastUpdated }: { probability: number; lastUpdated: Date } = $props();

	//if probability = 100 then deg = 90
	//if probability = 0 then deg = -90
	let deg = $derived((probability / 100) * 180 - 90);

	let color = $derived(probability > 50 ? '#c93136ff' : '#1675b7ff');
...
{#if size > 0}
	...
	>
		<div class="absolute z-50 text-center" style={`top: ${size / 2 + 32}px; width: ${size}px;`}>
			{#if probability > 50}
				<h1 class="font-serif text-3xl font-bold" style="color: {color}">Trump: {probability}%</h1>
			{:else}
				<h1 class="font-serif text-3xl" style="color: {color}">
					Comrade Kamala: {100 - probability}%
				</h1>
			{/if}
